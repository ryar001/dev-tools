...
if [[ "$DIFF_COMMAND" == "git diff --staged" ]]; then
    # For staged diff, include all files
    DIFF=$(git diff --staged -- . | grep -E '^(\+\+\+|---|\+|@@)' | grep -v '^[+ ]*$' | grep -v '^-*$' || true)
else
    # For commit range diff, include all files
    DIFF=$(eval "$DIFF_COMMAND" | grep -E '^(\+\+\+|---|\+|@@)' | grep -v '^[+ ]*$' | grep -v '^-*$' || true)
fi
